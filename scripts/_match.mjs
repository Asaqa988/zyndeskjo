import { course } from '../src/data/course/course.ts';
import type { Module } from '../src/data/course/types.ts';
...
export function matchAgainstCourse(analysis: CvAnalysis): CourseMatch {
  ...
  return {
    covered,
    modules,
    // Two distinct terms, or one term across two modules. One passing keyword
    // is a coincidence, not a reason to pitch a course.
    worthOffering: covered.length >= 2 || modules.length >= 2,
  };
}
